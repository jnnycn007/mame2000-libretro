...
#define XYTOL(val)	((((int32_t)((uint16_t)(val.y)) << state.xytolshiftcount1) |	\
				    (((int32_t)((uint16_t)(val.x))) << state.xytolshiftcount2)) + OFFSET)
...
{			  												\
	...
	int32_t a = R##REG(R##SRCREG) + (C_FLAG?1:0);				\
	int32_t *rd = &R##REG(R##DSTREG);							\
	int32_t b = *rd;											\
	int32_t r = *rd = a + b;									\
	SET_NZCV_ADD(a,b,r);									\
...
{															\
	...
	{														\
		...
		{													\
			...
			uint32_t signbits = ((quotient & 0x80000000) ? 0xffffffff : 0); 	\
...
{			  													\
	int32_t *rd = &R##REG(R##DSTREG);								\
	int32_t t = R##REG(R##SRCREG) + (C_FLAG?1:0);					\
	int32_t r = *rd - t;											\
	SET_NZCV_SUB(*rd,t,r);										\
	*rd = r;													\
	COUNT_CYCLES(1);											\
