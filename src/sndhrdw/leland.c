...
int leland_sh_start(const struct MachineSound *msound)
{
	...
	/* allocate memory */
	dac_buffer[0] = (uint8_t*)malloc(DAC_BUFFER_SIZE);
	dac_buffer[1] = (uint8_t*)malloc(DAC_BUFFER_SIZE);
...
void leland_dac_update(int dacnum, uint8_t *base)
{
